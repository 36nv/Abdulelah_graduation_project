...
if(isset($_POST['submit'])){
   ...
   $select = "SELECT * FROM user_form WHERE email = '$email'"; // امر الاستعلام الخاص  في قاعدة البيانات لاسترداد القييم
      ...
      if ($pass != $cpass) { //اذا كانت كلمة المرور غير متطابقة
         ...
         if (count($error) === 0) {// التحقق من عدم وجود اخطاء 
            ...
            if ($mail->send()) {
               ...
               $_SESSION['register_forwarding'] = true; //وادخله المستخدم بشكل صحيح  OTP يتم تخزين هذه القيمة كشرط أنه تم ارسال رمز  
               $_SESSION['pass'] = $pass;
